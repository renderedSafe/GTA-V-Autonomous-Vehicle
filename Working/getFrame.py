...
class GetFrameThread:
    def __init__(self, x=0, y=0, w=1920, h=1120, window_title_substring='PyCharm'):
        """
    ...
    def start(self):
        ...
        """
        time.sleep(1)
        Process(target=self.get_top, args=()).start()
        # Process(target=self._get_frame, args=(self.printscreen_middle,)).start()
        # Process(target=self._get_frame, args=(self.printscreen_bottom,)).start()
        # Process(target=self._stitch_images, args=()).start()

        return self

    def get_top(self):
        self.image = GrabScreen.get_frame()
